locals {
  docker_member_names = flatten([
    for key, list_value in var.docker_remote_proxy : "docker-remote-${key}"
  ])
}

locals {
  docker_blobstore_depenecies = lookup(var.s3, "enabled") ? nexus_blobstore_s3.docker_remote_blobstore_s3 : nexus_blobstore_file.docker_remote_blobstore
}

resource "nexus_blobstore_s3" "docker_remote_blobstore_s3" {
  name = "docker-remote"
  bucket_configuration {
    bucket {
      name       = "docker-remote"
      region     = lookup(var.s3, "region")
      expiration = lookup(var.s3, "expiration")
    }
    advanced_bucket_connection {
      endpoint         = lookup(var.s3, "url")
      force_path_style = lookup(var.s3, "path_style")
    }
    bucket_security {
      access_key_id     = lookup(var.s3, "access_key")
      secret_access_key = lookup(var.s3, "secret_key")
    }
  }
  count = lookup(var.s3, "enabled") ? 1 : 0

}
resource "nexus_blobstore_file" "docker_remote_blobstore" {
  name  = "docker-remote"
  path  = "docker-remote"
  count = lookup(var.s3, "enabled") ? 0 : 1
}

resource "nexus_repository_docker_proxy" "docker_proxy" {
  for_each = var.docker_remote_proxy

  name   = "docker-remote-${each.key}"
  online = true

  docker {
    force_basic_auth = false
    v1_enabled       = false
  }

  docker_proxy {
    index_type = each.value.index
  }

  storage {
    blob_store_name                = "docker-remote"
    strict_content_type_validation = true
  }

  proxy {
    remote_url       = each.value.url
    content_max_age  = 1440
    metadata_max_age = 1440
  }

  negative_cache {
    enabled = true
    ttl     = 1440
  }

  http_client {
    blocked    = false
    auto_block = true
  }

  depends_on = [
    local.docker_blobstore_depenecies
  ]

}

resource "nexus_repository_docker_group" "docker_remote_group" {

  name   = "docker-remote"
  online = true

  docker {
    force_basic_auth = false
    v1_enabled       = true
  }

  group {
    member_names = local.docker_member_names
  }

  storage {
    blob_store_name                = "docker-remote"
    strict_content_type_validation = true
  }

  depends_on = [
    local.docker_blobstore_depenecies,
    nexus_repository_docker_proxy.docker_proxy
  ]
}

resource "nexus_repository_docker_group" "docker_group" {

  name   = "docker-group"
  online = true

  docker {
    force_basic_auth = false
    v1_enabled       = true
  }

  group {
    member_names = local.docker_member_names
  }

  storage {
    blob_store_name                = "docker-remote"
    strict_content_type_validation = true
  }

  depends_on = [
    local.docker_blobstore_depenecies,
    nexus_repository_docker_proxy.docker_proxy
  ]
}
